...
impl TabsRecord {
    ...
    pub fn from_payload(payload: sync15::Payload) -> Result<Self> {
        ...
        let client_name: String =
            parse_string_from_json(&"clientName", payload.data.get("clientName"))?;
        let tabs: Result<Vec<TabsRecordTab>> = payload
            .data
            .get("tabs")
            ...
            .map(|x| -> Result<TabsRecordTab> {
                ...
                let url_history: Vec<String> = tabs_obj
                    ...
                    .into_iter()
                    .collect::<Result<_>>()?;
                let icon: Result<Option<String>, serde_json::Error> =
                    if let Some(icon) = tabs_obj.get("icon") {
                        if let Some(i) = icon.as_str() {
                            Ok(Some(i.to_string()))
                        } else {
                            Err(serde_json::Error::custom("invalid `icon`, expected string"))
                        }
                    } else {
                        Ok(None)
                    };
